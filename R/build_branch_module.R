...
build_branch_module <-
  function(dfClust,actuC,newC,scm,scm_nm,clStop,tfs,setGenesHC,goHC,gda,cliqueTargets,adjMethod,cutoff,qtarget,qprobInt,posRatio){
    ...
    if(!actuC %in% clStop){
      ...
      #Keep genes expressed at least in 10%
      ...
      if(length(colnames(nip)) > 10){
        ...
        cliqueRes <- find_mutual_cliques(as.matrix(nip),tfs,qtarget,qprobInt,posRatio) 
        ...
        if(length(cliqueRes) == 0){
          ...
          if(length(best_set_genes_go$FC)==0){
            ...
            if(length(which(colSums(mhc) == 0))==0){
              ...
              rc <- test_new_clique_from_list(scm,best_set_genes_go,incS=2,cellNum=10)
              use <- rc[[1]]
              mhc <- rc[[2]]
              top_set_genes <- rc[[3]]
              clNum <- rc[[4]]
              ...
              if(use){
                ...
                if(length(h)>1){
                  ...
                  if(length(unique(cl)) == 1){
                    ...
                    #If less than 10 cells but other cliques exists - try another one
                    rc <- test_new_clique_from_list(nip,best_set_genes_go,incS = 2,10)
                    usethen <- rc[[1]]
                    mhc <- rc[[2]]
                    top_set_genes <- rc[[3]]
                    if(usethen){
